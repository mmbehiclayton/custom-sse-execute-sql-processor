...
@ReadsAttributes({
        ...
        @ReadsAttribute(attribute = "operation.id", description = "Operation ID used for environment validation when validation is enabled.")
})
@WritesAttributes({
...
public class SseExecuteSQL extends AbstractExecuteSQL {
     ...
     * This property tells the processor which FlowFile attribute contains the operation ID.
     * The operation ID is used to look up environment configuration from the SSE Engine database.
     * Default value is "${operation.id}" which means it reads from the "operation.id" attribute.
     ...
     * This connects to the SSE Engine database (sse_engine schema) to query environment configurations.
     * This is where we look up which environment the operation belongs to and what the expected
     ...
     * This is the DBCP service that should connect to the source database according to the environment configuration.
     * We validate that this service's connection URL matches what's expected for the environment.
     ...
     * This is the DBCP service that should connect to the target database according to the environment configuration.
     * This service is used for BOTH validation AND the actual ExecuteSQL operation.
     * We validate that this service's connection URL matches what's expected for the environment.
     ...
     * This happens when the controller services don't match the expected environment configuration.
     ...
     * This includes the standard ExecuteSQL relationships plus our custom validation relationship.
    ...
    /**
     * Override the onTrigger method to add environment validation before ExecuteSQL processing.
     * This method is called by NiFi for each FlowFile that needs to be processed.
     * 
     * **CRITICAL BEHAVIOR**: When validation is enabled, SQL execution is GATED by validation results.
     * The SQL query will ONLY execute if validation passes. This is the core safety feature.
     * 
     * Processing Flow:
     * 1. Get the incoming FlowFile
     * 2. Check if validation is enabled
     * 3. If validation disabled → Call standard ExecuteSQL (super.onTrigger) - SQL executes normally
     * 4. If validation enabled → Perform environment validation
     * 5. If validation passes → Call standard ExecuteSQL - SQL executes
     * 6. If validation fails → Route to validation_failed (STRICT) or continue with warning (WARNING)
    ...
    public void onTrigger(ProcessContext context, ProcessSession session) throws ProcessException {
        ...
        // This is the master switch - when false, processor behaves like standard ExecuteSQL
        ...
        try {
            ...
            if (!validationResult.isValid()) {
                ...
                if ("STRICT".equals(validationMode)) {
                    ...
                    getLogger().warn("Environment validation warning for operation_id={}: {} - Proceeding with SQL execution anyway", 
                        validationResult.getOperationId(), validationResult.getErrorMessage());
                }
            } else {
     ...
     * This class holds environment configuration data retrieved from the SSE Engine database.
     ...
     * @return EnvironmentConfig object with all configuration details, or null if not found
    ...
    private EnvironmentConfig queryEnvironmentConfig(DBCPService engineDbcp, String operationId) 
        ...
        try {
            ...
            // This matches your working test queries and removes dependency on data_migration_records
            String sql = """
                SELECT DISTINCT
            ...
            if (!rs.next()) {
                // No results found - operation_id doesn't exist or has no environment configuration
